...
import json  # Import the json module for parsing ALLOWED_ORIGINS
...
ALLOWED_ORIGINS_STR = os.getenv("ALLOWED_ORIGINS", '["http://localhost:3000"]') #Use default value to stop application from breaking if the .env files fails.

try:
    ALLOWED_ORIGINS = json.loads(ALLOWED_ORIGINS_STR)  # Parse the JSON string
except json.JSONDecodeError as e:
    logger.error(f"Invalid JSON in ALLOWED_ORIGINS: {e}. Using default ['http://localhost:3000']")
    ALLOWED_ORIGINS = ["http://localhost:3000"]  # Provide a default value

REDIS_HOST = os.getenv("REDIS_HOST", "localhost")  # Default value
TESTING = os.getenv("TESTING", "False").lower() == "true"  # Convert to boolean